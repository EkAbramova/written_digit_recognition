...
class FNN():
    ...
    def __init__(self, weights, input_size=28 * 28, hidden_size=100, output_size=10):
        ...
        # Unpack variables from the params dictionary
            ...
            layer1 = X.dot(W1) + b1         # fully connected
            X2 = np.maximum(0, layer1)      # ReLU
            layer2 = X2.dot(W2) + b2        # fully connected

            # compute the class probabilities
            exp_scores = np.exp(layer2)
            probs = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)  # [N x K]

            # Backward pass: compute gradients
            grads = {}
            probs[range(X.shape[0]), y] -= 1
            dW2 = np.dot(layer1.T, probs)
            dW2 /= X.shape[0]
            dW2 += reg * W2
            grads['W2'] = dW2
